...
class AppTemplate
{
    ...
    var $page_id, $page_name, $page_enname, $page_size, $page_typearr,$page_type, $page_count, $data_count, $page,$startnum,$page_content;
    ...
    function getArtLink($id, $name, $enname, $typeid, $typename, $typeenname,$suffix)
    {
    ...
    function getArtPreNextLink($id,$flag)
    {
    	...
    	$row = $db->getRow("select a_id,a_title,a_entitle,a_type from {pre}art where 1=1 and a_hide=0 " .$where . " limit 0,1");
    	if ($row){
    		$tarr = getValueByArray($cache[1],"t_id" ,$row["a_type"]);
    		$str = "<span>".$str1.":<a href=". $this->getArtLink($row["a_id"],$row["a_title"],$row["a_entiele"],$row["a_type"],$tarr["t_name"],$tarr["t_enname"],true).">".$row["a_title"]."</a></span> ";
    	}
    	else{
    		$str = "<span>".$str1.":没有了</span> ";
    	}
    	unset($row);
    	return $str;
    }
    ...
    {
    	if (app_playtype ==1) { return $this->getVodPlayUrl($id,$name,$enname,$typeid,$typename,$typeenname,1,1); }
    ...
    function getVodPreNextLink($id,$flag)
    {
    	...
    	$row = $db->getRow("select d_id,d_name,d_enname,d_type from {pre}vod where 1=1 and d_hide=0 " . $where . " limit 0,1 ");
    	if (!$row){
    		$str = "<span>".$str1.":没有了</span> ";
    	}
    	else{
    		$tarr = getValueByArray($cache[0], "t_id" ,$row["d_type"]);
    		$str = "<span>".$str1.":<a href=". $this->getVodLink($row["d_id"],$row["d_name"],$row["d_enname"],$row["d_type"],$tarr["t_name"],$tarr["t_enname"]).">".$row["d_name"]."</a></span> ";
    	}
    	unset($row);
    	return $str;
    }
    ...
    function getPageLink($pagenum)
    {
        ...
        switch( $this->page_type )
        {
            ...
            case "label":
            	...
            	break;
            case "art":
            	$str = $this->getArtLink($this->page_id, $this->page_name, $this->page_enname,$this->page_typearr["t_id"],$this->page_typearr["t_name"],$this->page_typearr["t_enname"],false) . $this->getPageSuffix("art", app_artcontentviewtype,$pagenum);
            	break;
            default:
    ...
    function getPageListSizeByCache($strtype)
    {
    ...
    function typematrix()
    {
        ...
        for($i=0;$i<count($matches1[0]);$i++)
		{
            ...
            if($this->par_type=="letter"){
	            ...
	            $sql = "SELECT t_id,t_name,t_enname,t_key,t_des FROM {pre}" . $this->par_table . "_type WHERE t_pid=0 ";
				$where="";
				$orderstr="";
				$markhtml="";
    ...
    function menulist()
    {
        ...
        $sql = "SELECT t_id,t_name,t_enname,t_key,t_des FROM {pre}" . $this->par_table . "_type WHERE 0=0 ";
...
//        	var_dump($mac["vodtypeid"]);var_dump($mac["vodtypepid"]);
        	if($mac["vodtypepid"] ==='0'){
        		$where = $where . " AND t_pid = " . $mac["vodtypeid"];
        	}else if($mac["vodtypepid"] ==-1){
        		$where = $where . " AND t_pid = 0";
        	}else {
    ...
    function artlist()
    {
        ...
        $sql = "SELECT  a_id,a_title,a_subtitle,a_entitle,a_from,a_type,a_pic,a_author,a_color,a_hits,a_dayhits,a_weekhits,a_monthhits,a_addtime,a_time,".$field_des." FROM {pre}art WHERE a_hide=0 ";
    ...
    function vodlist()
    {
        ...
        $sql = $sql . "d_remarks,d_good,d_bad,d_score,d_scorecount,d_addtime,d_time,d_playfrom FROM {pre}vod WHERE d_hide=0 ";
        ...
        {   
            ...
            case "rnd": 
            	...
            	$sqlr = " {pre}vod AS t1 JOIN (SELECT ROUND(RAND() * ((SELECT MAX(d_id) as id FROM `{pre}vod` where d_type = ". $typearr['t_id'] .")-(SELECT MIN(d_id) as id FROM `{pre}vod` where d_type = ". $typearr['t_id'] ."))+(SELECT MIN(d_id) as id FROM `{pre}vod` where d_type = ". $typearr['t_id'] .")) AS id) AS t2  ";
            	$sql = replaceStr($sql,"{pre}vod",$sqlr);
            	$where .= " and t1.d_id >= t2.id  and t1.d_id !=".$typearr['d_id'] ." and t1.d_type = ". $typearr['t_id'] ;
//            	var_dump($typearr);
            	break;
            case "similar":
            	if ($this->par_similar !=""){
...
//        writetofile('d.txt',$sql . $where . $orderstr . $topstr);
		$rs = $db->query($sql . $where . $orderstr . $topstr );
		if ($rs){
			$rscount=$db->num_rows($rs);
		}
...
//            var_dump($sql . $where . $orderstr . $topstr);
    ...
    function topicpagelist()
    {
        ...
        for($i=0;$i<count($matches1[0]);$i++)
		{
            ...
            $sql = "SELECT t_id,t_name,t_enname,t_sort,t_template,t_pic,t_des FROM {pre}" . $this->par_table . "_topic WHERE 1=1 and t_userid=0 and t_id>4";
            if (!isNum($this->par_num)){ $this->par_num = 12;} else { $this->par_num = intval($this->par_num); }
    ...
    function yearlist()
    {
        ...
        if (!isNum($this->par_end) || strlen($this->par_end)<4) { $this->par_end=2012; }else {$this->par_end=intval($this->par_end);}
...
//                              }
    ...
    function pageshow()
	{
        ...
        for ($p=$i; $p<=$j; $p++){
            ...
            if ($p == $this->page){
                $strpagenum = $strpagenum . "<span class=\"current\">" . $p . "</span>&nbsp;";
            }
            else{
    ...
    function playlist($id, $name, $enname, $typeid, $typename, $typeenname, $playfrom, $playserver, $playurl,$webUrls)
    {
    	global $mac;
    	$playurlarrlen=0;
    	$weburlarrlen=0;
        ...
        if($webUrls !=""){
        	$weburlsarr = explode("$$$",$webUrls); $weburlarrlen = count($weburlsarr);
        }
        ...
        for($i=0;$i<count($matches1[0]);$i++)
		{
            ...
            for ($j=0;$j<$playfromarrlen;$j++){
                ...
                $tip = getVodXmlText("vodplay.xml","player", $playfromarr[$j] , 3);
//              
                ...
                if ($fromrc && $playurlarrlen >= $j){
                    ...
                    $urlarr = explode("#",$url);
                    $weburl = $weburlsarr[$j];
                    $weburlarr = explode("{Array}",$weburl);
                
                    if(!(isset($urlarr) && !is_null($urlarr)  && is_array($urlarr) && count($urlarr)>0 && strlen($urlarr[0])>0)){	                   
	                    $urlarrcount=0;
                    }else {
                       $urlarrcount = count($urlarr);
                    }
                      if(!(isset($weburlarr) && !is_null($weburlarr)  && is_array($weburlarr) && count($weburlarr)>0 && strlen($weburlarr[0])>0)){
	                        	continue;
	                    }	else {
	                    	 $weburlarrcount = count($weburlarr);
	                    }
	                    
	          
                    
//                   if($from !=='youku' && $from !=='letv' && $from !=='pptv' ){
//                   	  $urlarrcount=0;
//                   }
                   if($urlarrcount ==0 && $weburlarrcount==0){
                   	 continue;
                   }
                    ...
                    for($k=0;$k<count($matches3[0]);$k++)
					{
                        ...
                        $tempCount= $urlarrcount;
                        if($tempCount<$weburlarrcount){
                        	$tempCount=$weburlarrcount;
                        }
                        if ( strpos($markorder,"desc")>0){                        	
	                        for ($m=$tempCount;$m>=0;$m--){
	                        	$tUrl="";
	                        	if($m<$urlarrcount){
	                        		$tUrl=$urlarr[$m];
	                        	}else if($m<$weburlarrcount){
	                        		$tUrl=$weburlarr[$m];
	                        	}
	                            ...
	                            if (!isN($tUrl)){
	                                ...
	                                $markstr = replaceStr($markstr, "[urllist:link]", $this->getVodPlayUrl($id, $name, $enname, $typeid, $typename, $typeenname, $j + 1, ($m + 1)));
                        	...
                        	for ($m=0;$m<$tempCount;$m++){
	                            ...
	                            if (!isN($tUrl)){
	                              ...
	                              //  var_dump($weburlarrcount);
	                                ...
	                                $markstr = replaceStr($marktemp, "[urllist:num]", ($m + 1) );
	                                $urlname=replaceStr($urlname, "-", "/");
	                                $markstr = replaceStr($markstr, "[urllist:name]", $urlname);
	                                $markstr = replaceStr($markstr, "[urllist:path]", $urlpath);
	                                
	                                if($m >= $urlarrcount && ($from ==='tudou'|| $from ==='letv')){
	                                	$isWebUrlFlag=true;
	                                }
	                                if($isWebUrlFlag){
	                                	$urlpath='play.php?path='.base64_encode($urlpath).'&from='.$from;
	                                	$markstr = replaceStr($markstr, "[urllist:link]",$urlpath);
	                                }else {
	                                  $markstr = replaceStr($markstr, "[urllist:link]", $this->getVodPlayUrl($id, $name, $enname, $typeid, $typename, $typeenname, $j + 1, ($m + 1).'-'.$urlname));
	                                }
	                                $markslist2 = $markslist2 . $markstr;
	                            }
	                        }
                ...
                $markhtml = replaceStr($markhtml, "[playlist:urlcount]", $urlarrcount);
                $markhtml = replaceStr($markhtml, "[playlist:count]", $playfromarrlen);
                $markhtml = replaceStr($markhtml, "[playlist:num]", $show);
                $markhtml = replaceStr($markhtml, "[playlist:from]", $from);
                $markhtml = replaceStr($markhtml, "[playlist:show]", $show);
                $markhtml = replaceStr($markhtml, "[playlist:des]", $des);
                $markhtml = replaceStr($markhtml, "[playlist:tip]", $tip);
                $markhtml = replaceStr($markhtml, "[playlist:server]", $server);
                $markhtml = replaceStr($markhtml, "[playlist:serverdes]", $serverdes);
                $markhtml = replaceStr($markhtml, "[playlist:servertip]", $servertip);
                $markslist1 = $markslist1 . $markhtml;
            }
            unset($matches2);
            $this->html = replaceStr($this->html, $this->markval, $markslist1);
	    }
	    unset($urlarr);
	    unset($urlone);
	    unset($playurlarr);
    ...
    function downlist($id, $name, $enname, $typeid, $typename, $typeenname, $downurl)
    {
    	$downarr1len=0;
    	if ($downurl !=""){
        ...
        for($i=0;$i<count($matches1[0]);$i++)
		{
            ...
            for($j=0;$j<$downarr1len;$j++)
            {
                $num = $num + 1;
                $downarr2 = explode("$$",$downarr1[$j]);
                $from = $downarr2[0];
                
                $des = getVodXmlText("voddown.xml","down", $from , 2);
                $tip = getVodXmlText("voddown.xml","down", $from , 3);
	            ...
	            unset($matches3);
                $markhtml = replaceStr($markhtml, "[downlist:num]", $num);
                $markhtml = replaceStr($markhtml, "[downlist:from]", $from);
                $markhtml = replaceStr($markhtml, "[downlist:des]", $des);
                $markhtml = replaceStr($markhtml, "[downlist:tip]", $tip);
                $markslist1 = $markslist1 . $markhtml;
            }
            unset($matches2);
            $this->html = replaceStr($this->html, $this->markval, $markslist1);
    ...
    function downlists($id, $name, $enname, $typeid, $typename, $typeenname, $playfrom, $playserver, $playurl,$webUrls)
    {
    	global $mac;
    	$weburlarrlen=0;
        if($webUrls !=""){
...
//	                                $markslist2 = $markslist2 . $markstr;
                        	...
                        	for ($m=0;$m<$tempCount;$m++){
	                            ...
	                            if (!isN($tUrl)){
	                                ...
	                                $urlpath='download.php?path='.base64_encode($urlpath).'&from='.$from;
	                                $markstr = replaceStr($marktemp, "[urllist:num]", $m+1);
			                        $markstr = replaceStr($markstr, "[urllist:name]", $urlname);
			                        $markstr = replaceStr($markstr, "[urllist:path]", $urlpath);
			                        $markslist2 = $markslist2 . $markstr;
...
//                $markhtml = replaceStr($markhtml, "[playlist:urlcount]", $weburlarrcount);
//                $markhtml = replaceStr($markhtml, "[playlist:count]", $playfromarrlen);
//                $markhtml = replaceStr($markhtml, "[playlist:num]", $show);
//                $markhtml = replaceStr($markhtml, "[playlist:from]", $from);
//                $markhtml = replaceStr($markhtml, "[playlist:show]", $show);
...
//                $markhtml = replaceStr($markhtml, "[playlist:serverdes]", $serverdes);
//                $markhtml = replaceStr($markhtml, "[playlist:servertip]", $servertip);
                $markhtml = replaceStr($markhtml, "[downlist:num]", $show);
                $markhtml = replaceStr($markhtml, "[downlist:from]", $from);
                $markhtml = replaceStr($markhtml, "[downlist:des]", $des);
                $markhtml = replaceStr($markhtml, "[downlist:tip]", $tip);
                $markslist1 = $markslist1 . $markhtml;
    ...
    function replacePlayName($from,$url,$src,$num)
    {
	            ...
	            for ($j=0;$j<count($urlarr);$j++){
					if (!isN($urlarr[$j])){
						if ($j==($num-1)){
							...
							$this->html = replaceStr($this->html,"[playinfo:name]",$urlname);
