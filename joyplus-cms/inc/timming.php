...
if (app_timming==1){ 
	...
	foreach($timmingnodes as $timmingnode){
    	...
    	if (!isN($truntime)) { $oldweek= date('w',strtotime($truntime));
        $oldhours= date('H',strtotime($truntime)); }
		...
		if ($tstatus==1 && ( isN($truntime) || ($oldweek."-".$oldhours) != ($curweek."-".$curhours) && 
		strpos($tweeks,$curweek)>-1 && strpos($thours,$curhours)>-1)) {
			$timmingnode->getElementsByTagName("runtime")->item(0)->nodeValue = date('Y-m-d H:i:s');
			$doc -> save($xmlpath);
			if(strpos($tfile, "http") !==false){
				header("Location:" .$tfile."?".$tparamets  );
				writetofile("timming.txt", $tfile."?".$tparamets);
			}else {
			  header("Location:" . app_installdir."inc/".$tfile."?".$tparamets  );
			  writetofile("timming.txt", app_installdir."inc/".$tfile."?".$tparamets);
			}
			return;
		}
	}
