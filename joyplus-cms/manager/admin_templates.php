...
function info()
{
	...
	$file = be("get","file");
	$path = be("get","path");
	if (isN($backurl) || strpos($backurl,"index.php")>0){ $backurl="admin_templates.asp";}
...
<table class="tb">
		...
		while ($file=readdir($fso)){
			...
			if(is_file($fullpath)){
				if (substring($file,6)== "label_"){
					$fsize = round(filesize("$filedir/$file")/1024);
	...
	<?php
		...
		$upperFolder = substring($filedir,(strripos($filedir,"/")));
		if ($upperFolder !=".."){
