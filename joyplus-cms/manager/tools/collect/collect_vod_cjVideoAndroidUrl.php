    ...
    function parseVideoUrls($sql){
	    ...
	    if($rscount==0){		
	    	...
	    	while ($row = $db ->fetch_array($rs))	{
	    		...
	    		if(!isN($videoUrl)){
	    			$sql = "update {pre}cj_vod_url set iso_video_url='".$videoUrl."', u_weburl='".$u_weburl."' where u_id=".$u_id;
			        writetofile("parseVideo.txt", $sql);
			        $db->query($sql);
	    		}else {
...
//	    var_dump($rscount);
    ...
    function updateType($url,$project,$movieid,$p_id){
    	global $db;
    	$webCode = getPage($url,$project->p_coding);
    	//栏目设置
		if ($project->p_classtype ==1) {
			$typecode = filterScript(getBody($webCode,$project->p_typestart,$project->p_typeend),$project->p_script);
			$typecode = trim($typecode);
			$m_typeid = changeId($typecode,$p_id,0,0);
		}
		else{
			$typecode = $project->p_collect_type;
			$typecode = trim($typecode);
			$m_typeid = $project->p_collect_type;
		}
		$typecode = filterScript($typecode,$p_script);   		
		
		if(!isN($typecode)){
    		$sql = "update {pre}cj_vod set m_type='".$typecode."' where m_id=".$movieid;
	        writetofile("parseVideoTypesSQL.txt", $sql);
	        $db->query($sql);
    	}else {
    		writetofile("videoTypeErrors.txt", '{===}'.$p_id.'{===}'.$movieid.'{===}'.$url);
    	}	
    }
