...
//					echo $weburl;
					 ...
					 * get video url 
					 */
			         if($p_videocodeType ==3){ //api 
						$videoUrlParam = getBody($strViewCode,$p_videocodeApiUrlParamstart,$p_videocodeApiUrlParamend);
						$videoUrlParam = replaceLine($videoUrlParam);
						$p_videoUrlApi = replaceStr($p_videocodeApiUrl,"{PROD_ID}",$videoUrlParam);
						$videoUrlApiCode =getPageWindow($p_videoUrlApi,$p_coding);
						
						$videoAddressUrl = getBody($videoUrlApiCode,$p_videourlstart,$p_videourlend);
					 }else if($p_videocodeType ==2){//直接构造
						$videoUrlParam = getBody($strViewCode,$p_videourlstart,$p_videourlend);
						$videoAddressUrl = replaceStr($p_videocodeApiUrl,"{PROD_ID}",$videoUrlParam);
					 ...
					 }else if($p_videocodeType ==1){ //Base64Decode 获得视频地址
						$videoUrlParam = getBody($strViewCode,$p_videourlstart,$p_videourlend);
		...
		for ($i=0 ;$i< count($webArray);$i++){
			...
			if ($p_playlinktype == 1){ //播放页获取地址
			    ...
			    /**
					 ...
					 */
			          if($p_videocodeType ==3){ //api 
						$videoUrlParam = getBody($playCode,$p_videocodeApiUrlParamstart,$p_videocodeApiUrlParamend);
						$videoUrlParam = replaceLine($videoUrlParam);
						$p_videoUrlApi = replaceStr($p_videocodeApiUrl,"{PROD_ID}",$videoUrlParam);
						$videoUrlApiCode =getPageWindow($p_videoUrlApi,$p_coding);
						...
						$videoUrlParam = getBody($playCode,$p_videourlstart,$p_videourlend);
						$videoAddressUrl = replaceStr($p_videocodeApiUrl,"{PROD_ID}",$videoUrlParam);
						if(strpos($videoAddressUrl, MovieType::VIDEO_SEP_VERSION) !==false){
							$videoAddressUrls=MovieType::TOP_CLEAR.MovieType::VIDEO_NAME_URL_SEP.replaceStr($videoAddressUrl,MovieType::VIDEO_SEP_VERSION,MovieType::TOP_CLEAR).MovieType::VIDEO_SEP_VERSION;
						    $videoAddressUrls=$videoAddressUrls.MovieType::HIGH_CLEAR.MovieType::VIDEO_NAME_URL_SEP.replaceStr($videoAddressUrl,MovieType::VIDEO_SEP_VERSION,MovieType::HIGH_CLEAR).MovieType::VIDEO_SEP_VERSION;
...
//					 	writetofile("d:\\ss.txt",$p_videourlstart);
						$videoUrlParam = getBody($playCode,$p_videourlstart,$p_videourlend);	
					    if(!isN($videoUrlParam)){					
						    $videoUrls = explode("\",\"", $videoUrlParam);
						    if(isset($videoUrls) && is_array($videoUrls)){
...
//						writetofile("d:\\ss.txt",$videoAddressUrl);
					 }else {
					 	$videoAddressUrl="";
					 }  
			}
...
//					$url = $setname ."$" .$url;
			   ...
			   if (!$rowurl) {
				   if ($p_playlinktype ==1) {
					  $strTempUrl .=  $url . "<br>";
					  $url = replaceStr($url,"'","''");
					 writetofile("sql.txt","insert into {pre}cj_vod_url(u_url,u_movieid,u_weburl,iso_video_url,name) values('".$url."','".$movieid."','".getHrefFromLink($WebTestx)."','".getHrefFromLink($videoAddressUrl)."','".filterScriptStar($setname,$p_script)."')");
					  $db->query("insert into {pre}cj_vod_url(u_url,u_movieid,u_weburl,iso_video_url,name) values('".$url."','".$movieid."','".getHrefFromLink($WebTestx)."','".getHrefFromLink($videoAddressUrl)."','".filterScriptStar($setname,$p_script)."')");
					}
				   else{
					  $strTempUrl .= $url . "<br>";
					  writetofile("sql.txt","insert into {pre}cj_vod_url(u_url,u_movieid,iso_video_url,u_weburl) values('".$url."','".$movieid."','".getHrefFromLink($videoAddressUrl)."','".getHrefFromLink($strlink)."')");
					 
					  $db->query("insert into {pre}cj_vod_url(u_url,u_movieid,iso_video_url,u_weburl) values('".$url."','".$movieid."','".getHrefFromLink($videoAddressUrl)."','".getHrefFromLink($strlink)."')");
				   }
			   }
		}
	}
