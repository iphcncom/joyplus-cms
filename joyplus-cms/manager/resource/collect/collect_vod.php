...
<form id="form1" name="form1" action="?action=editsave" method="post">
	...
	<td>类别：</td>
	<td>
	<input id="m_type" name="m_type" type="text" value="<?php echo $m_type?>" size="100">
	</td>
	</tr>
    <tr>
	<td>图片：</td>
...
//       var_dump($videoUrls);
		...
		if(isN($tmpplayfrom)){
			...
			if (be("post","CCTV")=="2") {
				if(isN($videoUrls)){
					$strSet .="d_playfrom='".$tmpplayfrom . "$$$". $row["m_playfrom"]."',d_playserver='".$tmpplayfrom ."$$$".$row["m_playserver"]."',d_playurl='". $tmpplayurl ."$$$". $urls."' "."',webUrls='". $tmpweburl ."$$$". $webUrls."' ,d_downurl='". $tmpvideourl ."'";
				}else {
				  $strSet .="d_playfrom='".$tmpplayfrom . "$$$". $row["m_playfrom"]."',d_playserver='".$tmpplayfrom ."$$$".$row["m_playserver"]."',d_playurl='". $tmpplayurl ."$$$". $urls."' "."',webUrls='". $tmpweburl ."$$$". $webUrls."' ,d_downurl='". $tmpvideourl ."$$$".$row["m_playfrom"].'$$'. $videoUrls."'";  
			    }
			}
			else{
				...
				for ($k=0;$k<count($arr2);$k++){
					...
					if ($arr2[$k] === $row["m_playfrom"] ) { $tempWebArray1[$k] = $webUrls; }					
			   ...
			   for ($k=0;$k<count($tempVideoArray1);$k++){
					...
					if ($arr2[0] === $row["m_playfrom"] ) { 
						...
						if(isN($videoUrls)){
						    ...
						    $tempVideoArray1[$k] = $row["m_playfrom"].'$$'.$videoUrls;
//						     writetofile("d:\\up.txt", "here 1:".$tempVideoArray1[$k]."  .".$videoUrls);
						}
					}					
					$tmpvideourl = $tmpvideourl . $tempVideoArray1[$k];
//					writetofile("d:\\up.txt", "here 2:".$tmpvideourl);
					
				}
				if(!isN($videoUrls)){
					if(count($tempVideoArray1)==0){
						$tmpvideourl= $row["m_playfrom"].'$$'.$videoUrls."'";
	//					writetofile("d:\\up.txt", "here 3:".$tmpvideourl);
					}else {
						if(!$fromExis){
							$tmpvideourl=$tmpvideourl ."$$$".$row["m_playfrom"].'$$'. $videoUrls;
						}
			...
			if(isN($videoUrls)){
				$strSet .="d_playfrom='".$tmpplayfrom . "$$$". $row["m_playfrom"]."',d_playserver='".$tmpplayfrom ."$$$".$row["m_playserver"]."',d_playurl='". $tmpplayurl ."$$$". $urls."' ,webUrls='". $tmpweburl ."$$$". $webUrls ."' ";
			}else {
				$tempVideoArray1 = explode("$$$",$tmpvideourl );
				$tmpvideourl = "";
			    $rc = false;
