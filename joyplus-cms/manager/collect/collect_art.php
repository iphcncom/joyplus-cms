...
<?php
	...
	while ($row = $db ->fetch_array($rs))
	{
		...
		if (!$rowart){
			...
			$sql="insert {pre}art (a_type,a_addtime,a_time,a_content,a_hits,a_title,a_entitle,a_author) values('".$a_type."','".$a_addtime."','".$a_addtime."','".$a_content."','".$a_hits."','".$a_title."','".$a_entitle."','".$a_author."') ";
			$status = $db->query($sql);
			$aid = $db->insert_id();
		}
		//插入新数据结束
