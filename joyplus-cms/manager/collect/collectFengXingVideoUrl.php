...
require_once ("Project.php");
	...
	function updateLetvVideoUrl($pagenum,$endPage){	
	     global $db;
		 $sql = "SELECT count(*)  FROM {pre}vod WHERE webUrls IS not NULL and webUrls != ''  and d_hide =0 and d_type in (1,2,3,131)  and d_status = 0 and d_playfrom like '%fengxing%'  " ;
		 $nums = $db->getOne($sql);  
		 $app_pagenum=10; 
	...
	//	 $pagecount=2;
		 for($i=$pagenum;$i<=$pagecount&&$i<=$endPage;$i++){
		    ...
		    $sql = "SELECT webUrls,d_downurl, d_playfrom,d_id FROM {pre}vod WHERE webUrls IS not NULL and webUrls != '' and d_hide =0 and d_type in (1,2,3,131)   and d_playfrom like '%fengxing%' and d_status = 0 order by d_play_num desc limit ".($app_pagenum * ($i-1)) .",".$app_pagenum;
	//	    var_dump($sql);
		    $rs = $db->query($sql); 
		    parseVodPad($rs);
		    unset($rs);
...
//	          var_dump($url);
	          if(!isN($url)){
	          	$isovideoUrl = ContentProviderFactory::getContentProvider(ContentProviderFactory::FENG_XING)->parseIOSVideoUrl($url, "utf-8", '');
	    		$androidUrl = ContentProviderFactory::getContentProvider(ContentProviderFactory::FENG_XING)->parseAndroidVideoUrl($url, "utf-8", '');
