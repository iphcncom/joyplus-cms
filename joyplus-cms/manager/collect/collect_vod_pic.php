...
function updateVodPic($pagenum){	
	 ...
	 $sql = "SELECT count(*)  FROM {pre}vod WHERE (d_pic IS NULL OR d_pic = '' ) and d_type in (1,2) and d_douban_id !=-1" ;
	 $nums = $db->getOne($sql);  
	 $app_pagenum=10; 
...
//	 $pagecount=2;
	 for($i=$pagenum;$i<=$pagecount;$i++){
	    ...
	    $sql = "SELECT d_name,d_area, d_year,d_id,d_type, d_douban_id FROM {pre}vod WHERE (d_pic IS NULL OR d_pic = '') and d_type in (1,2)  and d_douban_id !=-1 order by d_type asc limit ".($app_pagenum * ($i-1)) .",".$app_pagenum;
//	    var_dump($sql);
	    $rs = $db->query($sql); 
        ...
        while ($row = $db ->fetch_array($rs))	{
            ...
            writetofile("updateVodThumb.txt", 'sssdoubanid{=}'.$doubanid );
			 ...
			 if(!isN($doubanid) && $doubanid !==0 && $doubanid !=='0' ){
			   $flag=false;writetofile("updateVodThumb.txt",'getThumb:'.$name);
		       $pic= $scoreDouban->getThumb($doubanid);
			 }else {
			 	 writetofile("updateVodThumb.txt",'getDoubanThumb:'.$name);
			 	$pic= $scoreDouban->getDoubanThumb($d_id,$name, $year, $area);
			 }
