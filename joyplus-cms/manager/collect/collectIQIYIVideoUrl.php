...
require_once ("Project.php");
	...
	function updateLetvVideoUrl($pagenum,$endPage){	
	     global $db;
		 $sql = "SELECT count(*)  FROM {pre}vod WHERE webUrls IS not NULL and webUrls != ''  and d_hide =0 and d_type in (1,2,3,131)  and d_status = 0 and d_playfrom like '%qiyi%'  " ;
		 $nums = $db->getOne($sql);  
		 $app_pagenum=10; 
	...
	//	 $pagecount=2;
		 for($i=$pagenum;$i<=$pagecount&&$i<=$endPage;$i++){
		 	writetofile("updateIqiyiVideoUrl.log", 'check item for vod type{=}'.$nums .'{=}Total{=}'.$pagecount.'{=}'.$i);
		    $sql = "SELECT webUrls,d_downurl, d_playfrom,d_id FROM {pre}vod WHERE webUrls IS not NULL and webUrls != '' and d_hide =0 and d_type in (1,2,3,131)   and d_playfrom like '%qiyi%' and d_status = 0  order by d_play_num desc limit ".($app_pagenum * ($i-1)) .",".$app_pagenum;
	//	    var_dump($sql);
		    $rs = $db->query($sql); 
		    parseVodPad($rs);
		    unset($rs);
//		    break;
    ...
    function parseOneVod($webUrls,$d_downurl,$d_playfrom,$d_id){
       global $db; 
       writetofile('updateIqiyiVideoUrl.log', 'd_id===='.$d_id);   		
	   ...
	   if(!isN($playurl)){
	       ...
	       foreach ($playurlArray as $nameUrls){
	          ...
	          writetofile('updateIqiyiVideoUrl.log', 'name===='.$name.'====url===='.$url); 
//	          var_dump($name);
//	          var_dump($url);
	          if(!isN($url)){
	          	$isovideoUrl = ContentProviderFactory::getContentProvider(ContentProviderFactory::QI_YI)->parseIOSVideoUrl($url, "utf-8", '');
	    		$androidUrl = ContentProviderFactory::getContentProvider(ContentProviderFactory::QI_YI)->parseAndroidVideoUrl($url, "utf-8", '');
...
//	       var_dump($videoUrl);
	   ...
	   if(!isN($videoUrl) && strpos($videoUrl, "http") !==false){
	   	  ...
	   	  writetofile('updateIqiyiVideoUrl.sql', $sql);
	   	  $db->query($sql);
	   }
