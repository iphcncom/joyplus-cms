...
<!--    &nbsp;<input type="button" id="btnNoin" class="btn" name="Submit" value="入库未入库" >-->
	...
	<input type="checkbox" name="CCTV9" value="9">影片长度
    <br />
...
//		    'name'=>$playNum,
        	...
        	if(isN($video['videoUrl'])&& $isMandCollect){			
			  sendMail(array('darkdotter.zhou@joyplus.tv','dale.wu@joyplus.tv'),'<CMS>:影片《'.$title.'》 视频地址为空,不能入库','<CMS>:影片《'.$title.'》 ===》 '.$video['name'].' 视频地址为空，播放器：'.$row["m_playfrom"]);
        	}else {
