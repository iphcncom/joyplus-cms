...
function save()
{
    ...
    $c_name = badFilter($c_name) ; $c_name = chkSql($c_name, false);
    $c_content = badFilter($c_content) ; $c_content = chkSql($c_content, false);
    $c_ip = getIP();
...
function main()
{
	...
	if (!isNum($c_vid) || !isNum($c_type)){ echo "err";exit;}
?>
<iframe width="100%" height="1" frameborder="0" scrolling="yes" src="<?php echo app_installdir?>plus/comment/?action=list&id=<?php echo $c_vid?>&type=<?php echo $c_type?>&title=" marginheight="0" marginwidth="0" name="comment" style="height: 350px;"></iframe>
<?php
}
	...
	if($nums==0){
		...
		while ($row = $db ->fetch_array($rs)){
			...
			while ($row1 = $db ->fetch_array($rs1)){
				...
				$replystr = $replystr . "<div class=\"citation\"><div class=\"citation-title\"><span class=\"author\"><span class=\"name\" style=\"color:".$bgstr.";\">".$c_name."</span><span class=\"address\">&nbsp;IP:[".$c_ip."]</span></span><span class=\"post-time\">".$c_time."</span><div class=\"clear\"><br/></div></div><p class=\"content\">".$c_content."</p></div>";
			}
			unset($rs1);
