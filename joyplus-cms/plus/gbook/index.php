...
function save()
{
    ...
    $g_name = badFilter($g_name); $g_name = chkSql($g_name, false);
    $g_content = badFilter($g_content); $g_content = chkSql($g_content, false);
    $g_ip = getIP();
...
?>
<iframe width="100%" height="1" frameborder="0" scrolling="yes" src="<?php echo app_installdir?>plus/gbook/?action=list&id=<?php echo $g_vid?>&name=<?php echo $g_vname?>" marginheight="0" marginwidth="0" name="gbook" style="height: 750px;"></iframe>
<?php
}
...
function glist()
{
	...
	if($nums==0){
		...
		while ($row = $db ->fetch_array($rs)){
			...
			if (!isN($g_reply)){
				$replystr = "<div class=\"citation\"><div class=\"citation-title\"><span class=\"author\"><span class=\"name\" style=\"color:".$bgstr.";\">管理员回复：</span><span class=\"address\"></span></span><span class=\"post-time\"></span><div class=\"clear\"><br/></div></div><p class=\"content\">".$g_reply."</p></div>";
			}
			$span="";
			if (strlen($g_name)<4 ){
			...
			echo "<div class=\"dt\"><span class=\"author\"><span class=\"name\" style=\"background-color:".$bgstr.";\">".$g_name.$span."</span><span class=\"address\">IP:[".$g_ip."]</span></span><span class=\"post-time\">发表于:[".$g_time."]</span><div class=\"clear\"><br/></div></div><div class=\"dd\">".$replystr."<p class=\"content\">".$g_content."</p><div class=\"toolbar\" id=\"toolbar-".$g_id."\"></div></div>";
