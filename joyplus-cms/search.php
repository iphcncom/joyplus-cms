...
<?php
    ...
    if ($mac["flag"]=="artsearch"){
	    ...
	    if (!isN($mac["letter"])){ $mac["key"]=$mac["letter"]; $mac["des"] = $mac["des"] . "&nbsp;首字母为" . $mac["letter"]; $mac["where"] = $mac["where"] . " AND a_entitle LIKE '" . $mac["letter"] . "%' ";}
	    ...
	    if (!isN($mac["letter"])){ $mac["key"]=$mac["letter"]; $mac["des"] = $mac["des"] . "&nbsp;首字母为" . $mac["letter"]; $mac["where"] = $mac["where"] . " AND d_enname LIKE '" . $mac["letter"] . "%' ";}
