...
writetofile("crawel_info.log", $p_id.'{=====}Project===start');

if (isN($p_id)) { writetofile("crawel_info.log", $p_id.'采集提示","采集项目ID不能为空!'); break; }
$tem=explode("_", $p_id);
if(count($tem)==2){
	$p_id=$tem[0];
	$pagenums=$tem[1];
...
function cjList()
{
	...
	global $reCollExistMovie, $p_playspecialtype,$starringarr,$titlearr,$picarr,$strdstate,$action,$p_pagebatchurl,$p_colleclinkorder,$p_id;
