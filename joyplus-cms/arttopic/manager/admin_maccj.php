...
$(document).ready(function(){
	...
	var status =false;
	if(fa!=""){ status=true; }
	getlist(rtype,rpage,rkey,status);
});
function setbtn()
{
	$("#cjsel").click(function(){
	    ...
	    $("#form3").attr("action","admin_maccj.php?action=cjsel&xt="+xt+'&rid='+rid+"&cjflag="+cjflag+"&rpage="+rpage+ "&rtype="+rtype);
	    $("#form3").submit();
	});
	$("#cjday").click(function(){
		if(xt=="1" || xt=="2"){ rday="24"; } else { rday="1"; }
	    $("#form3").attr("action","admin_maccj.php?action=cjday&rday="+rday+"&xt="+xt+'&rid='+rid+"&cjflag="+cjflag+"" );
	    $("#form3").submit();
	});
	$("#cjtype").click(function(){
	    if (rtype=="0"){ alert("请先进入分类,否则无法使用采集分类"); return;}
	    $("#form3").attr("action","admin_maccj.php?action=cjtype&rtype="+rtype+"&xt="+xt+'&rid='+rid+"&cjflag="+cjflag);
	    $("#form3").submit();
	});
	$("#cjall").click(function(){
	    $("#form3").attr("action","admin_maccj.php?action=cjall&xt="+xt+'&rid='+rid+"&cjflag="+cjflag );
	    $("#form3").submit();
	});
}
function getlist(t,p,k,c)
{
	rpage=p;
	rtype=t;
	rkey=k;
	$.ajax({
        cache: c, dataType: 'html', type: 'GET', url:  'admin_maccj.php?action=glist&xt='+xt+'&rid='+rid+'&cjflag='+cjflag+'&rpage='+p+'&rtype='+t+'&rkey='+ encodeURI(k) +'&cjurl='+cjurl,
...
function glist()
{
    ...
    if ($xt == "1"){
		...
		$xn_vod = '/<video><last>([\s\S]*?)<\/last><id>([0-9]+)<\/id><tid>([0-9]+)<\/tid><name><\!\[CDATA\[([\s\S]*?)\]\]><\/name><type>([\s\S]*?)<\/type><dt>([\s\S]*?)<\/dt><note><\!\[CDATA\[([\s\S]*?)\]\]><\/note>/';
		$xn_vod_id=2;
		$xn_vod_name = 4;
	    $xn_vod_starring=7;
	    $xn_vod_type=5;
    ...
    echo "<div class='nav'><ul><li style=\"width:70px;\"><a href=\"javascript:void(0)\" onclick=\"getlist(0,1,rkey,true);\"><font color=\"#FF0000\"><b>全部资源</b></font></a></li>";
    ...
    foreach($array2[1] as $key=>$value){
        ...
        $typebind .= "</a>";
        echo "<li><a href='javascript:void(0)' onclick=\"getlist(".$typeid.",1,rkey,true);\">" . $typename . "</a>" .$typebind . "</li>";
    }
    unset($array2);
    echo "    </ul> </div>";
    ...
    echo "<tr> <td colspan=\"2\">  全选<input name=\"chkall\" type=\"checkbox\" id=\"chkall\" value=\"1\" onClick=\"checkAll(this.checked,'ids[]');\"/>&nbsp;  <input type='button' id='cjsel' value='采集选中'/>  <input type='button' id='cjday' value='采集当天'/> <input type='button' id='cjtype'value='采集当前分类'/>   <input type='button' id='cjall' value='采集全部'/>   </td>  </tr>   <tr>   <td colspan=\"5\" align=\"center\">". pagelist_manage($vodpagecount, $rpage, $vodrecordcount, $vodpagesize, "'javascript:void(0)' onclick=\"getlist(".$rtype.",{p},'".$rkey."',true);\"") ." </td>   </tr></table></form>";
}
...
<p>绑定完毕后点击 <a href="?action=list&xt=<?php echo $xt?>&rid=<?php echo $rid?>&cjflag=<?php echo $cjflag?>&cjurl=<?php echo $cjurl?>">【进入采集数据列表】 </a> 。</p>
<p>本地分类列表：</p>
<div class='nav'>
<ul>
<?php
...
{
    global $db;
	$sql = "select * from {pre}vod_type where t_union is not null ";
	$rs= $db->query($sql);
	while ($row = $db ->fetch_array($rs))
	{
		$unionids = $row["t_union"];
		$unionarr = explode(",",$unionids);
		for ($i=0 ;$i< count($unionarr);$i++){
			if (trim($unionarr[$i]) == $servertype){
				$result = $row["t_id"];
				return $result;
				break;
			}
