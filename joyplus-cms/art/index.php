<?php
    ...
    if (app_artcontentviewtype == 0 || app_vodviewtype == 3){
    ...
    if ($page < 1){ $page = 1;}
    $sql = "SELECT a_id,a_title,a_subtitle,a_entitle,a_from,a_type,a_content,a_author,a_color,a_hits,a_dayhits,a_weekhits,a_monthhits,a_addtime,a_time,a_pic FROM {pre}art WHERE a_hide=0 and a_id=" . $id;
    $row = $db->getRow($sql);
