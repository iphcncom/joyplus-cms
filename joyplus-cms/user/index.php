...
function chklogin()
{
	global $db,$u_plays,$u_fav,$u_points;
...
function checkLogin()
{
	...
	if (!$row){
		...
		if ($row["u_flag"] == 1){
			if ( time() <= strtotime($row["u_end"]) )  {  $ugroup = app_reggroup;} else{ $ugroup=$row["u_group"]; }
		}
		else{
			$ugroup=$row["u_group"];
		}
		...
		$db->Update ("{pre}user",array("u_logintime","u_ip","u_random","u_loginnum","u_group"),array(date('Y-m-d H:i:s'),getIP(),$randnum,$row["u_loginnum"] + 1,$ugroup),"u_id=".$row["u_id"]);
...
function reg()
{
	...
	if ($ref=="qqlogin"){
		...
		if($qc->check_oauth_token()){
			$arr = $qc->get_user_info();
			$qqid = $qc->qqid;
			$nickname = $arr["nickname"]; $nickname=chkSql($nickname,true); $nickname=replaceStr($nickname,"'","");
			$tmpname = $nickname;
...
<div id="sideSub">
    ...
    <div class="footer">
	<div class="link_bk">&copy; 2008-2012 maccms.com, All Rights Reserved. 苹果CMS 版权所有</div>
	<div class="link_bk" style="font-size:10px;"> Powered by <a href="http://www.maccms.com/" target="_blank" style="color:#009900"><em> maccms v7.x</em></a></div>
	</div>
</div>
</div>
<?php
...
function playsdel()
{
	global $db,$u_plays;
	$d_id = be("arr","d_id");
	$d_id=chkSql($d_id,true);
	if (! isN($d_id)){
		$arr = explode(",",$d_id);
		for ($i=0;$i<count($arr);$i++){
			if (strpos(",".$u_plays,",". trim($arr[$i]) .",") > 0){
				$u_plays = replaceStr($u_plays, trim($arr[$i]),"");
				$u_plays = replaceStr($u_plays,",,",",");
			}
		}
		$db->Update  ("{pre}user" ,array("u_plays"), array($u_plays) ,"u_id="& $_SESSION["userid"] );
	}
	alerturl ("删除完毕!","?action=plays");
}
...
<div id="content">
	...
	<div id="footer">
	<div class="copyright">&copy; 2008-2012 maccms.com, All Rights Reserved. 苹果CMS 版权所有</div>
	</div>
</div>
</div>
<?
...
<td width="15%" class="us_list_title">消费积分</td>
	...
	<?php
		...
		$sql = "SELECT d_id,d_name,d_enname,d_type,d_stint FROM {pre}vod  where d_id in (". $u_plays.") order by d_addtime desc ";
	...
	<?php
		...
		$rsr = $db->getRow("SELECT t_name,t_enname FROM {pre}vod_type WHERE t_id=". $row["d_type"] );
		if (!$rsr){
			echo  "未知";
		}
		else{
			...
			$tid=$row["d_type"];
			$tname=$rsr["t_name"];
			$tenname=$rsr["t_enname"];
		}
		unset($rsr);
	?>
		</td>
		<td><a target="_blank" href="<?php echo $template->getVodLink($row["d_id"],$row["d_name"],$row["d_enname"],$tid,$tname,$tenname)?>">浏览</a></td>
...
<div class="user_right_box">
	...
	<?php
		...
		$sql = "SELECT d_id,d_name,d_enname,d_type,d_stint FROM {pre}vod  where d_id in (". $u_fav.") order by d_time desc ";
		$sql .= " limit ".(30 * ($pagenum-1)) .",30";
		$rs = $db->query($sql);
	...
	<tr>
		...
		<td><a target="_blank" href="<?php echo $template->getVodLink($row["d_id"],$row["d_name"],$row["d_enname"],$row["d_type"],$tname,$tenname)?>">浏览</a></td>
		<td><?php echo $row["d_stint"]?></td>
	</tr>
	<?php
		}
...
<div class="user_right_box">
	...
	<table width="100%" border="0" cellpadding="4" cellspacing="0" style="margin-top:20px; margin-bottom:20px;" >
		<tr>
		<td width="150" height="30" align="right">当前所在会员组：</td>
		<td> <?php echo $row["ug_name"]?>
		</td>
		</tr>
		<tr>
		<td width="150" height="30" align="right">升级到会员组：</td>
		<td>提升等级：<select id="u_group" name="u_group">
	    ...
	    <tr align="center">
	      <td colspan="2"><input class="input" type="submit" value="保存" id="btnSave"> <input class="input" type="button" value="返回" id="btnCancel1"> </td>
	    </tr>
	</table>
</form>
</div>
...
<div class="user_right_box">
	...
	<table width="100%" border="0" cellpadding="4" cellspacing="0" style="margin-top:20px; margin-bottom:20px;" >
	    ...
	    <tr align="center">
	      <td colspan="2"><input class="input" type="submit" value="确认" id="btnSave"> <input class="input" type="button" value="返回" id="btnCancel2"> </td>
	    </tr>
	</table>
	</form>
</div>
